"""
inference.py — Real-time Turkish Sign Language Recognition (v2)
Improved: lower thresholds, faster response, better display
"""
...
# ── Constants ─────────────────────────────────────────────────────────────────
WINDOW_SIZE  = 64
STRIDE       = 4       # predict every 4 frames
CONF_THRESH  = 0.50    # lowered from 0.70
STABLE_COUNT = 1       # commit immediately
IDLE_THRESH  = 0.008   # very sensitive motion detection
NUM_CLASSES  = 226
...
def prepare_input(frame_buffer):
    ...
    return torch.cat([x_tensor, velocity], dim=0).unsqueeze(0)  # (1, 6, 64, 56)
...
def draw_hud(frame, gloss_sequence, current_sign, confidence, is_active, top3):
    ...
    if current_sign and is_active:
        ...
        # Main
        color = (0, 255, 100) if confidence > 0.70 else (255, 200, 0)
...
def main():
    ...
    model = STGCN(num_classes=NUM_CLASSES, dropout=args.dropout).to(device)
    ckpt  = torch.load(args.checkpoint, map_location=device, weights_only=False)
    model.load_state_dict(ckpt["model"])
    model.eval()
    print(f"Model loaded. Best val_acc: {ckpt.get('val_acc', 'N/A')}")
    ...
    with mp_holistic.Holistic(
        ...
        while True:
            ...
            mp_drawing.draw_landmarks(
                ...
                mp_drawing.DrawingSpec(color=(200, 80, 0),  thickness=1))
            mp_drawing.draw_landmarks(
            ...
            if len(frame_buffer) == WINDOW_SIZE and frame_count % STRIDE == 0:
                input_tensor = prepare_input(frame_buffer).to(device)
            ...
            cv2.imshow("TID - Turkish Sign Language", frame)
